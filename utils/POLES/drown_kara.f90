...
SUBROUTINE cslf(varin,lsm,nx,ny,varout)

  IMPLICIT NONE

  REAL(8),DIMENSION(nx,ny),INTENT(IN)  :: varin, lsm
  ...
  INTEGER                              :: nt, nmax=100 ! customizable
  ...
  nt = nt + 1
  PRINT *, 'number of iteration : ', nt
  PRINT *, 'number of filled points :' , SUM(zmask) , 'on ' , nx * ny
